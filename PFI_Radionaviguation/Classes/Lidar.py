...
class Lidar:
    ...
    def demarrer_scan(self):
        if self.lidar.turnOn():
            self.activer = True
        time.sleep(1.5) # Attend un peut que le lidar soit prêt avant de commencer le scan
    ...
    def detecter_obstacle(self, largeur_angle, distance):
        if self.activer and self.lidar.doProcessSimple(self.scan) and len(self.scan.points) > 0:
            
            angle_radian = (math.radians(largeur_angle)) / 2
            seuil_avant = math.pi - angle_radian

            for point in self.scan.points:
                if point.range > self.PORTEE_MAX or point.range == 0:
                    continue

                portee = point.range
                angle = point.angle

                angle = point.angle + math.radians(self.OFFSET_LIDAR)
                if angle > math.pi:  angle -= 2 * math.pi
                if angle < -math.pi: angle += 2 * math.pi

                if portee <= distance:
                    if angle > seuil_avant or angle < -seuil_avant:
                        print("Obstacle détecté à", portee, "mètres, angle:", math.degrees(angle))
                        return True
