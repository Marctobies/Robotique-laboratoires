...
from Classes.Radionavigation import Radionavigation
import cv2
import threading
import time

class Robot:
    SEUIL_CORRECTION_ANGLE = 1
    MINIMUM_VITESSE_TOURNANT = 0.6
    ...
    def gauche(self, duree=0.01):
        ...
        vitesse_tournant = max(self.vitesse, self.MINIMUM_VITESSE_TOURNANT)
        self.moteur_droit.reculer(vitesse_tournant)
        self.moteur_gauche.avancer(vitesse_tournant)
        sleep(duree)

    def droite(self, duree=0.01):
        self.changer_etat(Etats.ROTATION_DROITE)
        vitesse_tournant = max(self.vitesse, self.MINIMUM_VITESSE_TOURNANT)
        self.moteur_droit.avancer(vitesse_tournant)
        self.moteur_gauche.reculer(vitesse_tournant)
        sleep(duree)
    ...
    def tourner_angle(self, degrees_tournant):
        angle_actuel = self.orientation.obtenir_angle()
        angle_cible = angle_actuel + degrees_tournant
        ecart = angle_cible - angle_actuel
        while abs(ecart) >= self.SEUIL_CORRECTION_ANGLE:            
            if ecart > 0:
                self.gauche()
            else:
                self.droite()
    ...
    def avancer_sur_axe(self, axe, indice):
        if axe == 'x':
            selecteur = 0
        else:
            selecteur = 1
        ...
        while depart is None:
            depart = self.radionavigation.obtenir_position()[selecteur]
        actuel = depart
        cible = depart + indice
        if indice > 0:
            while actuel < cible:
                print(actuel)
                if self.lidar.detecter_obstacle(50, 0.5):
                    self.freiner()
                    time.sleep(0.01)
                else:
                    self.avancer()
                actuel = self.radionavigation.obtenir_position()[selecteur]
        else:
            while actuel > cible:
                print(actuel)
                if self.lidar.detecter_obstacle(50, 0.5):
    ...
    #---------ROUTINE MOUVEMENT--------#
    #----------------------------------#
    def tracer_rectangle(self, distance_x=0.5, distance_y=0.75):
        self.lidar.demarrer_scan()
        self.avancer_sur_axe('x', distance_x)
        self.tourner_angle(90)
        self.avancer_sur_axe('y', distance_y)
        self.tourner_angle(90)
        self.avancer_sur_axe('x', -distance_x)
        self.tourner_angle(90)
        self.avancer_sur_axe('y', -distance_y)
    ...
    """
    def tourner_angle(self, degrees_tournant):
        angle_cible = self.orientation.obtenir_angle() + degrees_tournant

        if degrees_tournant > 0:
            while self.orientation.obtenir_angle() < angle_cible:
                self.gauche() 
        else:
            while self.orientation.obtenir_angle() > angle_cible:
                self.droite()

        print("Angle atteint:", self.orientation.obtenir_angle())
        self.freiner()
